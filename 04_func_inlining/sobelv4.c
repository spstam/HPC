...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i=1; i<SIZE-1; i+=1) {
		for (j=1; j<SIZE-1; j+=1 ) {
			...
			help_p_horiz = input[(i -1)*SIZE + j -1] * horiz_operator[0][0];
        	help_p_horiz += input[(i -1)*SIZE + j] * horiz_operator[0][1];
        	help_p_horiz += input[(i -1)*SIZE + j + 1] * horiz_operator[0][2];
        	//i=-1 j=-1,0,1
        	help_p_horiz += input[(i)*SIZE + j -1] * horiz_operator[1][0];
        	help_p_horiz += input[(i)*SIZE + j] * horiz_operator[1][1];
        	help_p_horiz += input[(i)*SIZE + j + 1] * horiz_operator[1][2];
        	//i=-1 j=-1,0,1
        	help_p_horiz += input[(i + 1)*SIZE + j -1] * horiz_operator[2][0];
        	help_p_horiz += input[(i + 1)*SIZE + j] * horiz_operator[2][1];
        	help_p_horiz += input[(i + 1)*SIZE + j + 1] * horiz_operator[2][2];


			help_p_vert = input[(i -1)*SIZE + j -1] * vert_operator[0][0];
        	help_p_vert += input[(i -1)*SIZE + j] * vert_operator[0][1];
        	help_p_vert += input[(i -1)*SIZE + j + 1] * vert_operator[0][2];
        	//i=-1 j=-1,0,1
        	help_p_vert += input[(i)*SIZE + j -1] * vert_operator[1][0];
        	help_p_vert += input[(i)*SIZE + j] * vert_operator[1][1];
        	help_p_vert += input[(i)*SIZE + j + 1] * vert_operator[1][2];
        	//i=-1 j=-1,0,1
        	help_p_vert += input[(i + 1)*SIZE + j -1] * vert_operator[2][0];
        	help_p_vert += input[(i + 1)*SIZE + j] * vert_operator[2][1];
        	help_p_vert += input[(i + 1)*SIZE + j + 1] * vert_operator[2][2];
			
			p = pow(help_p_horiz, 2) + pow(help_p_vert,2);
