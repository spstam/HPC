...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i=1; i<SIZE-1; i+=1) {
		for (j=1; j<SIZE-1; j+=1 ) {
			...
			//helper to avoid +/i ops with j
                ...
                //hor
                help_p_horiz -= input[input_idx];
                //0
                help_p_horiz += input[input_idx + 2];

                //vert
                help_p_vert = input[input_idx];
                help_p_vert += input[input_idx + 1] << 1;
                help_p_vert += input[input_idx + 2];
                
                input_idx += SIZE;

                //hor
                help_p_horiz -= (input[input_idx] << 1);
                //0
                help_p_horiz += input[input_idx + 2]  << 1;
                //vert 
                //0 because its 0
                //hor 
			...
			//omit unnecessary sqrt ops
			 if (p > 65025)
				...
				output[output_idx] = (unsigned char)sqrt(p);
	...
	output_idx = SIZE +1;	
	 for (i=1; i<SIZE-1; i++) {
		for ( j=1; j<SIZE-1; j++ ) {
			t = pow((output[output_idx] - golden[output_idx]),2);
			PSNR += t;
			output_idx += 1;
		}
		output_idx+=2;
